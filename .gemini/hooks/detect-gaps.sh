...
if [ -d "Assets/Scripts" ]; then
  SRC_CHECK=$(find Assets/Scripts -type f \( -name "*.gd" -o -name "*.cs" -o -name "*.cpp" -o -name "*.c" -o -name "*.h" -o -name "*.hpp" -o -name "*.rs" -o -name "*.py" -o -name "*.js" -o -name "*.ts" \) 2>/dev/null | head -1)
  if [ -n "$SRC_CHECK" ]; then
    FRESH_PROJECT=false
  fi
fi
...
if [ -d "Assets/Scripts" ]; then
  ...
  SRC_FILES=$(find Assets/Scripts -type f \( -name "*.gd" -o -name "*.cs" -o -name "*.cpp" -o -name "*.c" -o -name "*.h" -o -name "*.hpp" -o -name "*.rs" -o -name "*.py" -o -name "*.js" -o -name "*.ts" \) 2>/dev/null | wc -l)
else
  SRC_FILES=0
fi
...
if [ "$SRC_FILES" -gt 50 ] && [ "$DESIGN_FILES" -lt 5 ]; then
  ...
  echo "    Suggested action: /reverse-document design Assets/Scripts/[system]"
...
if [ -d "Assets/Scripts/Core" ] || [ -d "src/engine" ]; then
  if [ ! -d "docs/architecture" ]; then
    ...
    if [ "$ADR_COUNT" -lt 3 ]; then
      ...
      echo "    Suggested action: /reverse-document architecture Assets/Scripts/Core/[system]"
    fi
  fi
fi
  ...
  if [ -n "$GAMEPLAY_SYSTEMS" ]; then
    while IFS= read -r system_dir; do
      ...
      if [ "$file_count" -ge 5 ]; then
        # Check for design doc (case-insensitive search)
        design_match=$(find design/gdd -type f -iname "*${system_name}*" 2>/dev/null | head -1)

        if [ -z "$design_match" ]; then
          echo "⚠️  GAP: Gameplay system 'Assets/Scripts/Gameplay/$system_name/' ($file_count files) has no design doc"
          echo "    Expected: design/gdd/*${system_name}*.md"
          echo "    Suggested action: /reverse-document design Assets/Scripts/Gameplay/$system_name"
        fi
      fi
