...
| **Gemini CLI** | AI agent CLI | `npm install -g @anthropic-ai/gemini-code` |
...
- Hooks use `bash .gemini/hooks/[name].sh` — this works on Windows because
  Gemini CLI invokes commands through a shell that can find `bash.exe`
...
Gemini CLI works with any editor, but the template is optimized for:
- **VS Code** with the Gemini CLI extension
- **Cursor** (Gemini CLI compatible)
- Terminal-based Gemini CLI CLI
