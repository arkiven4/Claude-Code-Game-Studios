...
This documents the JSON payloads each Gemini CLI hook receives on stdin for every event type.
...
Fired when a Gemini CLI session begins. **No stdin input** — the hook just runs and its stdout is shown to Gemini as context.
...
Fired when the Gemini CLI session ends. **No stdin input** — the hook runs for cleanup and logging.
